...
def character_answer(character, question):
    if question in character['answers']:
        return character['answers'][question] #вот здесь кажется не хватает функции с неважными чертами
    return 0.5

def calculate_character_probability(character, questions_so_far, answers_so_far):
    # Prior
    P_character = 1 / len(characters) #вот эта вещь должна быть заменена на столбец с процентами покрытия
